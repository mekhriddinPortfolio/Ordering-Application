...
class DateTimePicker: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
  ...
  // Reference from https://stackoverflow.com/questions/40878547/is-it-possible-to-have-uidatepicker-work-with-start-and-end-time
    ...
    var didSelectDates: ((_ start: Date, _ day: Date) -> Void)?
  ...
  func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
    ...
    let startDay = days[dayIndex]
    
    didSelectDates?(startTime, startDay)
  }
...
extension Date {
    ...
    static func buildTimeRangeString(startDate: Date, startDay: Date) -> String {
    
//    let dayFormatter = DateFormatter()
//    dayFormatter.dateFormat = "E, d MMM yyyy HH:mm:ss Z"
//      return dayFormatter.string(from: startDate)
//
      
      
      let dayFormatter = DateFormatter()
      dayFormatter.dateFormat = "E, d MMM yyyy"

      let startTimeFormatter = DateFormatter()
      startTimeFormatter.dateFormat = "HH:mm:ss Z"
      
      return "\(dayFormatter.string(from: startDay)) \(startTimeFormatter.string(from: startDate))"
