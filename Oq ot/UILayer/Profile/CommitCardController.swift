...
class CommitCardController: BaseViewController {
    ...
    var titleLabel: UILabel = {
       let lbl = UILabel()
        ...
        lbl.layer.backgroundColor = Theme.current.titleLabelColor.cgColor
    ...
    lazy var nextButton: BaseButton = {
        ...
        b.setAttributedTitle(NSAttributedString.getAttrTextWith(18, "confirmButton".translate(), false, UIColor(hex: "#000000", alpha: 0.3)), for: .normal)
            ...
            guard let self = self else {return}
              if self.secondsRemaining > 0 {
                  Utils.delay(seconds: 0) {
                      self.sendSmsAgainButton.setAttributedTitle(NSAttributedString.getAttrTextWith(15, "\("canSentAgain".translate()) \(self.secondsRemaining) \("sentAgainTimer".translate())"), for: .normal)
                  }
                  self.secondsRemaining -= 1
              } else {
                  self.sendSmsAgainButton.setAttributedTitle(NSAttributedString.getAttrTextWith(15, "sentAgain".translate()), for: .normal)
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if !(string == "") {
            ...
            if !TF1.text!.isEmpty && !TF2.text!.isEmpty && !TF3.text!.isEmpty && !TF4.text!.isEmpty && !TF5.text!.isEmpty && !TF6.text!.isEmpty  {
                self.nextButton.setAttributedTitle(NSAttributedString.getAttrTextWith(18, "confirmButton".translate(), false, UIColor(hex: "#FFFFFF", alpha:1.0)), for: .normal)
                self.nextButton.layer.backgroundColor = UIColor(hex: "#DE8706").cgColor
                self.nextButton.isUserInteractionEnabled = true
            }
            return false
        }
        self.nextButton.setAttributedTitle(NSAttributedString.getAttrTextWith(18, "confirmButton".translate(), false, UIColor(hex: "#000000", alpha:0.3), .center), for: .normal)
